...
import { ErrorResponse, PipUUID, MessageResponse} from "@bluedotrobots/common-ts"
...
export default async function confirmUserPreviouslyAddedUUID(
	...
	try {
		...
		if (uuidUserRecord === false) {
			res.status(400).json({ message: "User hasn't registered this UUID" } as MessageResponse)
			return
		}
		next()
	} catch (error) {
