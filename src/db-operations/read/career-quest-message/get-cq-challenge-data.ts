...
export default async function getCQCareerData(
	userId: number,
	careerId: number
	...
	try {
		...
		const [chats, allSubmissions, allHints, sandboxes] = await Promise.all([
			...
			prismaClient.career_quest_chat.findMany({
				where: {
			...
			prismaClient.career_quest_code_submission.findMany({
				where: {
					challenge_id: { in: challengeIds },
					user_id: userId
				},
			...
			prismaClient.career_quest_hint.findMany({
				where: {
					challenge_id: { in: challengeIds },
					user_id: userId
				},
			...
			prismaClient.career_quest_sandbox.findMany({
				where: {
					challenge_id: { in: challengeIds },
					user_id: userId
				},
		...
		const results: CareerQuestChallengeData[] = challenges.map(challenge => {
			...
			if (!isNil(sandbox) && sandbox.career_quest_sandbox_json) {
				try {
